...
function get_latest_height() {
  ...
  height=`fbchaincli status --node http://${IP}:${port} | jq .sync_info.latest_block_height | awk '{ gsub(/"/,""); print $0 }'`
  echo $height
}
...
function add_val() {
  index=$1
  node_name=val${index}
  seed_addr=$(fbchaind tendermint show-node-id --home cache/node0/fbchaind)@${IP}:${seedp2pport}
  echo "add val >>> "$node_name

  fbchaind_opts="--p2p.allow_duplicate_ip  --p2p.pex=false  --p2p.addr_book_strict=false  --consensus.timeout_commit 600ms    --upload-delta=false  --elapsed DeliverTxs=0,Round=1,CommitRound=1,Produce=1  --consensus-role=v${index}  --p2p.seeds ${seed_addr} "

  start_node $index $node_name $fbchaind_opts
}
...
function add_seed() {
  ...
  fbchaind_opts="--p2p.seed_mode=true  --p2p.allow_duplicate_ip  --p2p.pex=false  --p2p.addr_book_strict=false  --consensus.timeout_commit 600ms  --upload-delta=false  --elapsed DeliverTxs=0,Round=1,CommitRound=1,Produce=1  --consensus-role=v$index "
...
function add_rpc() {
  ...
  fbchaind_opts="--p2p.seeds ${seed_addr} "
  start_node $index $node_name $fbchaind_opts
}
...
function case_1() {
  ...
  #STEP kill rpc
  killbyname_gracefully "cache/node4/fbchaind"
  sleep 2
...
function case_2() {
  ...
  #STEP upgrade 100% v
  killbyname_gracefully "cache/node2/fbchaind"
  sleep 3
  add_val 2

  killbyname_gracefully "cache/node1/fbchaind"
  sleep 3
  add_val 1

  killbyname_gracefully "cache/node0/fbchaind"
  sleep 3
  add_seed
  ...
  #STEP kill rpc
  killbyname_gracefully "cache/node4/fbchaind"
  killbyname_gracefully "cache/node5/fbchaind"
  sleep 3
